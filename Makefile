...
tensor-eval-gpu: tensor_main_gpu.o Makefile
	$(NVCC) $(GPU_OPTS) -o tensor-eval-gpu -I . -DGPU -L ${SPLATT_LIBRARY_DIR} -lcuda -lcudart -L ${CUDA_LIBRARY_DIR} -lcusparse tensor_main_gpu.o gpu_kernels.cu -lpthread -lm -ldl -lsplatt -liomp5 -lblas -llapack

tensor_main_gpu.o: tensor_main.cpp Makefile
	$(NVCC) $(GPU_OPTS) -o tensor_main_gpu.o $(CC_FLAGS) -I . -I ${EIGEN_INCLUDE_DIR} -I ${SPLATT_INCLUDE_DIR} -lcuda -lcudart -I ${CUDA_INCLUDE_DIR} tensor_main.cpp
