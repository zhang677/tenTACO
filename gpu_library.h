...
void copy_to_device_spmspv(taco_tensor_t *y, taco_tensor_t *A, taco_tensor_t *x) {
  ...
  /*
  gpuErrchk(cudaMemcpyToSymbolAsync(global_y_vals_device, &global_y_vals_host_copy, sizeof(double*)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_A2_pos_device, &global_A2_pos_host_copy, sizeof(int*)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_A2_crd_device, &global_A2_crd_host_copy, sizeof(int*)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_A_vals_device, &global_A_vals_host_copy, sizeof(double*)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_x_vals_device, &global_x_vals_host_copy, sizeof(double*)));
 
  gpuErrchk(cudaMemcpyToSymbolAsync(global_x_pos_device, &global_x_pos_host_copy, sizeof(int*)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_x_crd_device, &global_x_crd_host_copy, sizeof(int*)));

  gpuErrchk(cudaMemcpyToSymbolAsync(global_y1_dimension, &num_rows, sizeof(int)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_A1_dimension, &num_rows, sizeof(int)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_x_dimension, &nx, sizeof(int)));
  */
}
...
void copy_to_device_spmv(taco_tensor_t *y, taco_tensor_t *A, taco_tensor_t *x) {
  ...
  gpuErrchk(cudaMemcpyToSymbolAsync(global_y1_dimension, &num_rows, sizeof(int)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_A1_dimension, &num_rows, sizeof(int)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_x_dimension, &num_cols, sizeof(int)));
  */
}
...
/*
typedef struct taco_matrix_d_host{
  int32_t*     dimensions;
  float*     vals;   
}taco_matrix_d_host;
typedef struct taco_matrix_s_host{
  uint8_t***   indices;
  int32_t*     dimensions;
  double*     vals;   
}taco_matrix_s_host;

taco_matrix_d_host* spmm_dense_C;
taco_matrix_d_host* spmm_dense_B;
taco_matrix_s_host* spmm_sparse_A;
*/
// TODO: change the double(CUDA_R_64F) to float(CUDA_R_32F)
...
/*
  printf("start!\n");
  gpuErrchk(cudaMalloc(&spmm_dense_B, sizeof(taco_matrix_d_host)));
  gpuErrchk(cudaMalloc(&spmm_dense_C, sizeof(taco_matrix_d_host)));
  gpuErrchk(cudaMalloc(&spmm_sparse_A, sizeof(taco_matrix_s_host)));
  gpuErrchk(cudaMalloc(&(spmm_dense_B->dimensions), 2 * sizeof(int32_t)));
  gpuErrchk(cudaMalloc(&(spmm_dense_B->vals), num_j * num_k * sizeof(float)));
  gpuErrchk(cudaMalloc(&(spmm_dense_C->dimensions), 2 * sizeof(int32_t)));
  gpuErrchk(cudaMalloc(&(spmm_dense_C->vals), num_i * num_k * sizeof(float)));
  gpuErrchk(cudaMalloc(&(spmm_sparse_A->indices), 2 * sizeof(uint8_t**)));
  gpuErrchk(cudaMalloc(&(spmm_sparse_A->indices[1]), 2 * sizeof(uint8_t*)));
  gpuErrchk(cudaMalloc(&(spmm_sparse_A->indices[1][0]), (num_i + 1) * sizeof(uint8_t)));
  gpuErrchk(cudaMalloc(&(spmm_sparse_A->indices[1][1]), nnz * sizeof(uint8_t)));
  gpuErrchk(cudaMalloc(&(spmm_sparse_A->dimensions), 2 * sizeof(int32_t)));
  gpuErrchk(cudaMalloc(&(spmm_sparse_A->vals), nnz * sizeof(float)));

  gpuErrchk(cudaMemcpyAsync(spmm_dense_B->vals, B_vals, num_j * num_k * sizeof(float), cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpyAsync(spmm_dense_C->vals, C_vals, num_i * num_k * sizeof(float), cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpyAsync(spmm_sparse_A->indices[1][0], A2_pos, (num_i + 1) * sizeof(uint8_t), cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpyAsync(spmm_sparse_A->indices[1][1], A2_crd, nnz * sizeof(uint8_t), cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpyAsync(spmm_sparse_A->vals, A_vals, nnz * sizeof(float), cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpyAsync(spmm_dense_B->dimensions, B_dimensions, 2 * sizeof(int32_t), cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpyAsync(spmm_dense_C->dimensions, C_dimensions, 2 * sizeof(int32_t), cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpyAsync(spmm_sparse_A->dimensions, A_dimensions, 2 * sizeof(int32_t), cudaMemcpyHostToDevice));
  printf("here!\n");
*/
...
void copy_from_device_spmm(taco_tensor_t *C, taco_tensor_t *A, taco_tensor_t *B) {
  ...
  */
  ...
  //gpuErrchk(cudaMemset(global_C_vals_host_copy, 1.0, (num_i * num_k * sizeof(float))));

  gpuErrchk(cudaMemcpyAsync(C_vals, global_C_vals_host_copy, (num_i * num_k * sizeof(float)), cudaMemcpyDeviceToHost));
  //gpuErrchk(cudaMemcpyAsync(C_vals, spmm_dense_C->vals, (num_i * num_k * sizeof(float)), cudaMemcpyDeviceToHost));
  //gpuErrchk(cudaMemcpyAsync(B_vals, global_B_vals_host_copy,  num_j * num_k * sizeof(float), cudaMemcpyDeviceToHost));
  //gpuErrchk(cudaMemcpyAsync(A_vals, global_A_vals_host_copy, nnz * sizeof(float), cudaMemcpyDeviceToHost));
...
void copy_to_device_sddmm(taco_tensor_t *A, taco_tensor_t *B, taco_tensor_t *C, taco_tensor_t *D) {
  ...
  // copy from host_copy to symbol device
  /*
  gpuErrchk(cudaMemcpyToSymbolAsync(global_D_vals_device, &global_D_vals_host_copy, sizeof(double*)));
  ...
  gpuErrchk(cudaMemcpyToSymbolAsync(global_B_vals_device, &global_B_vals_host_copy, sizeof(double*)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_C_vals_device, &global_C_vals_host_copy, sizeof(double*)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_A_vals_device, &global_A_vals_host_copy, sizeof(double*)));
  

  gpuErrchk(cudaMemcpyToSymbolAsync(global_D1_dimension, &num_i, sizeof(int)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_B1_dimension, &num_j, sizeof(int)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_A1_dimension, &num_j, sizeof(int)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_C2_dimension, &num_k, sizeof(int)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_C1_dimension, &num_j, sizeof(int)));
  gpuErrchk(cudaMemcpyToSymbolAsync(global_D2_dimension, &num_k, sizeof(int)));
  */
}
...
/*    
  gpuErrchk(cudaFree(spmm_dense_B->dimensions));
  gpuErrchk(cudaFree(spmm_dense_B->vals));
  gpuErrchk(cudaFree(spmm_dense_C->dimensions));
  gpuErrchk(cudaFree(spmm_dense_C->vals));
  gpuErrchk(cudaFree(spmm_sparse_A->dimensions));
  gpuErrchk(cudaFree(spmm_sparse_A->vals));
  gpuErrchk(cudaFree(spmm_sparse_A->indices[1][1]));
  gpuErrchk(cudaFree(spmm_sparse_A->indices[1][0]));
  gpuErrchk(cudaFree(spmm_sparse_A->indices[1]));
  gpuErrchk(cudaFree(spmm_sparse_A->indices));
  gpuErrchk(cudaFree(spmm_dense_B));
  gpuErrchk(cudaFree(spmm_dense_C));
  gpuErrchk(cudaFree(spmm_sparse_A));
*/
