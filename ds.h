...
taco_tensor_t to_taco_tensor(const splatt_kruskal factored, const splatt_csf* tt, int mode) {
  mode = tt->dim_perm[mode];

  taco_tensor_t mt;

  mt.dimensions = new int32_t[2];
  mt.dimensions[0] = factored.dims[mode];
  mt.dimensions[1] = factored.rank;
  mt.vals = (float*)factored.factors[mode];
