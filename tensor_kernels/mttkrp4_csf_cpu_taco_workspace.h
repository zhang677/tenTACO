#ifndef MTTKRP4_CPU_TACO_WORKSPACE_H
#define MTTKRP4_CPU_TACO_WORKSPACE_H

#include "ds.h"
#include "timers.h"

void mttkrp4_cpu_taco_workspace(taco_tensor_t *A, taco_tensor_t *B, taco_tensor_t *C, taco_tensor_t *D, taco_tensor_t *E) {
TIME_COLD(
  ...
  for (splatt_idx_t i1 = 0; i1 < ((B1_dimension + 63) / 64); i1++) {
    double* restrict BDE_workspace = 0;
    BDE_workspace = (double*)malloc(sizeof(double) * C2_dimension);
    for (splatt_idx_t pBDE_workspace = 0; pBDE_workspace < C2_dimension; pBDE_workspace++) {
      BDE_workspace[pBDE_workspace] = 0.0;
    }
    double* restrict BE_workspace = 0;
    BE_workspace = (double*)malloc(sizeof(double) * C2_dimension);
    for (splatt_idx_t pBE_workspace = 0; pBE_workspace < C2_dimension; pBE_workspace++) {
      BE_workspace[pBE_workspace] = 0.0;
    }
        ...
        for (splatt_idx_t lB = B3_pos[kB]; lB < B3_pos[(kB + 1)]; lB++) {
          ...
          for (splatt_idx_t mB = B4_pos[lB]; mB < B4_pos[(lB + 1)]; mB++) {
            ...
            for (splatt_idx_t j = 0; j < C2_dimension; j++) {
              ...
              BE_workspace[j] = BE_workspace[j] + B_vals[mB] * E_vals[jE];
            }
          }
          for (splatt_idx_t j = 0; j < C2_dimension; j++) {
            splatt_idx_t jD = l * D2_dimension + j;
            BDE_workspace[j] = BDE_workspace[j] + BE_workspace[j] * D_vals[jD];
            BE_workspace[j] = 0;
          }
        }
        for (splatt_idx_t j = 0; j < C2_dimension; j++) {
          ...
          A_vals[jA] = A_vals[jA] + BDE_workspace[j] * C_vals[jC];
          BDE_workspace[j] = 0;
        }
      }
    }
