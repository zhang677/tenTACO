...
__global__
void mttkrp3_gpu_ebsr_kernel(taco_tensor_t * __restrict__ A, taco_tensor_t * __restrict__ B, taco_tensor_t * __restrict__ C, taco_tensor_t * __restrict__ D, int32_t* i_blockStarts, int32_t* k_blockStarts){
  int A2_dimension = global_A2_dimension;
  float* A_vals = global_A_vals_device_float;
  int* B1_pos = global_B1_pos_device;
  int* B1_crd = global_B1_crd_device;
...
void mttkrp_32_eb_sr_512_4_1(taco_tensor_t *A, taco_tensor_t *B, taco_tensor_t *C, taco_tensor_t *D) {
  ...
  int32_t* k_blockStarts = 0; // 64 = nnz_per_block
  gpuErrchk(cudaMallocManaged((void**)&k_blockStarts, sizeof(int32_t) * ((B3_pos[B2_pos[B1_pos[1]]] + 63) / 64 + 1)+1048576));
  ...
  TIME_GPU(
    ...
    gpuErrchk(cudaMemset(global_A_vals_host_copy_float, 0, ((size_t) A1_dimension * A2_dimension) * 4));
    mttkrp3_gpu_ebsr_kernel<4,1><<<(B3_pos[B2_pos[B1_pos[1]]] + 63) / 64, 512>>>(A, B, C, D, i_blockStarts, k_blockStarts);
  );
  cudaDeviceSynchronize();
  gpuErrchk(cudaGetLastError());
  cudaFree(k_blockStarts);
...
__global__
void mttkrp3_gpu_ebpr_kernel(taco_tensor_t * __restrict__ A, taco_tensor_t * __restrict__ B, taco_tensor_t * __restrict__ C, taco_tensor_t * __restrict__ D, int32_t* i_blockStarts, int32_t* k_blockStarts){
  int A2_dimension = global_A2_dimension;
  float* A_vals = global_A_vals_device_float;
  int* B1_pos = global_B1_pos_device;
  int* B1_crd = global_B1_crd_device;
...
void mttkrp_32_eb_pr_256_64_32_manual(taco_tensor_t *A, taco_tensor_t *B, taco_tensor_t *C, taco_tensor_t *D) {
  ...
  TIME_GPU(
    ...
    gpuErrchk(cudaMemset(global_A_vals_host_copy_float, 0, ((size_t) A1_dimension * A2_dimension) * 4));
    mttkrp3_gpu_ebpr_kernel<64,8,32><<<(B3_pos[B2_pos[B1_pos[1]]] + 63) / 64, 256>>>(A, B, C, D, i_blockStarts, k_blockStarts);
  );
  cudaDeviceSynchronize();
  gpuErrchk(cudaGetLastError());
  cudaFree(k_blockStarts);
