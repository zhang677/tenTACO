#ifndef MTTKRP3_CPU_TACO_UNSCHEDULED_H
#define MTTKRP3_CPU_TACO_UNSCHEDULED_H

#include "ds.h"
#include "timers.h"

void mttkrp3_cpu_taco_unscheduled(taco_tensor_t *A, taco_tensor_t *B, taco_tensor_t *C, taco_tensor_t *D) {
TIME_COLD(
  ...
  for (splatt_idx_t i = 0; i < B1_dimension; i++) {
    for (splatt_idx_t pB2 = B2_pos[i]; pB2 < B2_pos[(i + 1)]; pB2++) {
      ...
      for (splatt_idx_t pB3 = B3_pos[pB2]; pB3 < B3_pos[(pB2 + 1)]; pB3++) {
        ...
        for (splatt_idx_t j = 0; j < D2_dimension; j++) {
